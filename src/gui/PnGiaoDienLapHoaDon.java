...
public class PnGiaoDienLapHoaDon extends JPanel implements ActionListener, MouseListener {
	...
	public PnGiaoDienLapHoaDon() {
		...
		ctsddv_dao = new ChiTietSuDungDichVu_DAO();
		...
		ctsddvhd_dao = new ChiTietSuDungDichVuHoaDon_DAO();
...
//		Tự động load số tiền thùa
	...
	public void actionPerformed(ActionEvent e) {
			...
			if (selectedRow != -1) {
				for (int i = 0; i < tblTTHD.getRowCount(); i++) {
					if (modelTTHD.getValueAt(i, 1).toString().contains("Phòng")) {
						...
						ChiTietPhieuDatPhong ctpdp = ctpdp_dao.getChiTietPhieuDatPhongTheoMa(pdpTheoTTHD.getMaPDP());
...

		{
			...
			if (tblHD.getSelectedRow() != -1) {
				FrmChonKhuyenMai frmKM = new FrmChonKhuyenMai(this, lblTenKM, lblGTKM, txtTongTien);
	...
	private void docDuLieuPhongChuaThanhToan() {
	...
	private void docDuLieuChiTietDichVuSD() {
	...
	private void loadLoaiDichVuVaoCombobox() {
	...
	private void loadDichVuVaoComboboxTheoLoai(String tenLoaiDichVu) {
	...
	private void capNhapSoLuongTonKhoDichVu() {
		int selectedRow = tblHD.getSelectedRow();
		if (selectedRow != -1) {
			for (int i = 0; i < tblTTHD.getRowCount(); i++) {
				if (modelTTHD.getValueAt(i, 1).toString().contains("Phòng")) {
	...
	private void timKiemHoaDonTheoTenPhong() {
		String tenPhong = txtTKPhong.getText();
		Phong phong = phong_dao.getPhongTheoTen(tenPhong);
	...
	private void timKiemHoaDonTheoTenKhachHang() {
	...
	private void timKiemHoaDonTheoSĐTKH() {
		String sdt = txtSDT.getText();
