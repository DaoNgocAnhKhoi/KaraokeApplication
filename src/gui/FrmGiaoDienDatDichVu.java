...
public class FrmGiaoDienDatDichVu extends JFrame implements ActionListener {
	...
	public FrmGiaoDienDatDichVu(ArrayList<Phong> dsp) {
...
//			 TODO Auto-generated catch block
	...
	private void updateComboTenDV(String tenLoaiDV) {
...
//					xóa hết chi tiết sử dụng dịch vụ để phục vụ cho việc nếu phòng đó đã đặt dịch vụ trước đó giờ muốn cập nhật lại
				...
				for (int i = 0; i < model.getRowCount(); i++) {
					String maDV = dv_dao.getDichVuTheoTen(model.getValueAt(i, 0).toString()).getMaDichVu();
					...
					LoaiDichVu loaiDV = dv_dao.getDichVuTheoTen(model.getValueAt(i, 0).toString()).getLoaiDichVu();
					String hinhAnh = dv_dao.getDichVuTheoTen(model.getValueAt(i, 0).toString()).getHinhAnh();
					DichVu dv = new DichVu(maDV, tenDV, dv_dao.getDichVuTheoTen(tenDV).getSoLuong(), donVi, donGia,
							loaiDV, hinhAnh);
...
//	Đọc dữ liệu nếu phòng đó đặt dữ liệu trước đó
	private void docDuLieuVaoBang(ArrayList<Phong> dsp) {
		PhieuDatPhong pdp = pdp_dao.getPhieuDatPhongTheoMaPhong(dspht.get(0).getMaPhong());
